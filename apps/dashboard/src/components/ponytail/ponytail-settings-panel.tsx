...
export default function PonytailSettingsPanel() {
  ...
  useEffect(() => {
    ...
    setMinInputTokens(String(data.min_input_tokens ?? 12000));
    setMinMessages(String(data.min_messages ?? 16));
    setProtectedWindow(String(data.protected_window ?? 8));
    setCompressionConversation(data.compress_conversation);
    setCompressionCode(data.compress_code);
    setCompressionDeduplicate(data.deduplicate);
  }, [data]);

  const save = async () => {
    setSaving(true);
    try {
      const payload: PonytailSettings = {
        enabled,
        mode,
        min_input_tokens: Number(minInputTokens),
        min_messages: Number(minMessages),
        protected_window: Number(protectedWindow),
        compress_conversation: compressionConversation,
        compress_code: compressionCode,
        deduplicate: compressionDeduplicate,
        metadata,
      };
      await api.putPonytailSettings(payload);
